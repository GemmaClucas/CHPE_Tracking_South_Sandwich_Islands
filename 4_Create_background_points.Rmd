...
### Create a raster layer for the distance from the colony
```{r}
colony_lat<- -57.808 
colony_lon<- -26.404
...
# basic contour plot but does not give labels
bathy_mask %>% as.data.frame(xy = TRUE) %>% 
  ggplot(aes(x = x, y = y, z = full_ssi18a)) +
  geom_contour()

# version using metR package, but currently borken
library(metR)
...
Make a raster which includes the distance to the shelf break. Haven't finished this yet, still need to calculate distances.
```{r}
contours <- rasterToContour(bathy_mask)
plot(bathy_mask)
plot(contours, add=TRUE)
```


you have the depth layer which you downloaded when you were here. 
from that you can extract different contours using the rasterToContour funtion. 
i think it is the gdistance function to calculate a distance raster from a polygon (ie distance to shelf break) but I cant find it anywhere in my code
