...
```{r}
#first identify the nearest bit of shelf break using the contour line and the distance from colony raster created above
costsum <- 
rm(costsum)
  raster::extract(dist,contour500) %>% 
  unlist() %>% 
  min()
#dist is the distance from the colony raster 
unl<-unlist(costsum) 
UN<-unl[!is.na(unl)] 
G<-min(UN)#THIS IS THE VALUE of the raster AT THE MINIMUM DISTANCE TO SHELF 
```
