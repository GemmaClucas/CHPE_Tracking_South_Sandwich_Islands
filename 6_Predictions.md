...
I’ve added a line to change predicted probabilities to zero if the
distance is greater than 100km from the colony. We know from other
studies that chick-provision chinstraps don’t forage this far from the
colony in general, and only one trip was further than 100km from the
colony out of the 388 total chick-rearing trips that we recorded in this
study.
...
the distributions around Thule, Cook, and Bellingshausen overlap so much
-\> have to create a combined distance raster for distances from all
colonies and predict from this.
