...
This just creates a raster across the study area.
```{r}
...
Crop out land from the study area raster using ```mask()``` and change the resolution to a 1x1km grid.
```{r}
...
# Plot to check
plot(mask)
plot(mask, xlim = c(-26.6, -26.2), ylim = c(-57.9, -57.7))
```

Not sure why the edges of the study area are bumpy in the zoomed out map... something to do with projections??

Also note that this is a 1.02km x 1km grid - is that a problem?
...
Make a new raster with a 1km x 1km resolution. This is what we will use to sample the distance and SST rasters. I got the values for the max and min long/lat by converting the depth raster to LAEA after cropping it to the study area extent.
```{r}
long <- seq(-120531.7, 84896.27, 1000) #first minimum longitude in m, then max, 1000 is 1km
head(long)
lat <- seq(-50280.75, 81319.25, 1000)
head(lat)
Saunders_1KmPoints <- expand.grid(long,lat)
names(Saunders_1KmPoints) <- c("Lon","Lat")
coordinates(Saunders_1KmPoints) <- ~Lon+Lat
# reproject it back to wgs84 to extract the environmentla data
proj4string(Saunders_1KmPoints) <- CRS("+proj=laea +lon_0=-26 +lat_0=-58 +units=m")
Saunders_1KmPoints <- spTransform(Saunders_1KmPoints, CRS = CRS("+proj=longlat +ellps=WGS84"))
...
Make predictions - NEED TO FIGURE OUT HOW TO PLOT THIS.
```{r}
Saunders_1KmPoints$GAM_pred <- as.numeric(predict(GAM, type="response", newdata = Saunders_1KmPoints))

# ggplot(data = data.frame(Saunders_1KmPoints), aes(x = Lon, y = Lat)) + 
#   stat_density2d()
