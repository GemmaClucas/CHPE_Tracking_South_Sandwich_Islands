...
![](5_GAMs_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
    ...
    ## s(colonydist) 2.00 2.00    0.96  <2e-16 ***
    ## s(SST)        5.00 3.99    0.87  <2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

The residuals look pretty good in the plots on the left. The plots on
the right are always going to look weird because it’s binomial I
think.
...
Removing the cubic spline with shrinkage `(bs = 'cs')` does not seem to
alter the
result.
...
```
    ...
    ## Parametric coefficients:
    ##             Estimate Std. Error z value Pr(>|z|)    
    ## (Intercept)  -0.9249     0.1782  -5.189 2.11e-07 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## Approximate significance of smooth terms:
    ##                 edf Ref.df Chi.sq p-value    
    ## s(colonydist) 1.997  2.000  30623  <2e-16 ***
    ## s(SST)        4.047  4.132   9974  <2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## R-sq.(adj) =  0.585   Deviance explained = 50.3%
...
Removing the double penalty `select = TRUE` does seem to allow the
number of basis functions for colonydist and SST to increase very
slightly within the bounds I have set, therefore keep it in going
forwards.

### What about a different way to do model selection?

Here I was playing around with the model selection method, to see
whether GVC.Cp or REML does a better job. To use these methods, you have
to run the full model and then let it determine which covariates are
important and how many wiggles they should have. I have switched to the
bam method as it’s quicker to compute for large
datasets.
...
```
    ...
    ## 
    ## Formula:
    ## pres ~ s(colonydist) + s(SST) + s(Height) + s(slope) + s(NorthVelocity) + 
    ##     s(EastVelocity) + s(chlorA) + s(depth)
    ## 
    ## Parametric coefficients:
    ##             Estimate Std. Error z value Pr(>|z|)    
    ## (Intercept)  -1.2061     0.1984  -6.079 1.21e-09 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## Approximate significance of smooth terms:
    ##                    edf Ref.df  Chi.sq p-value    
    ## s(colonydist)    7.924      9 9271.59  <2e-16 ***
    ## s(SST)           5.166      9  103.31  <2e-16 ***
    ## s(Height)        8.999      9 1101.24  <2e-16 ***
    ## s(slope)         6.472      9   99.11  <2e-16 ***
    ## s(NorthVelocity) 7.912      9 1241.58  <2e-16 ***
    ## s(EastVelocity)  7.671      9  324.02  <2e-16 ***
    ## s(chlorA)        7.926      9  688.61  <2e-16 ***
    ## s(depth)         7.994      9 1608.10  <2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## R-sq.(adj) =  0.619   Deviance explained = 54.8%
    ## UBRE = -0.10222  Scale est. = 1         n = 170699

``` r
gratia::draw(BAM, residuals = TRUE)
```

![](5_GAMs_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
...
```
    ...
    ## Parametric coefficients:
    ##             Estimate Std. Error z value Pr(>|z|)    
    ## (Intercept)  -0.6075     0.0447  -13.59   <2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## Approximate significance of smooth terms:
    ##                    edf Ref.df  Chi.sq p-value    
    ## s(colonydist)    7.876      9 9396.63  <2e-16 ***
    ## s(SST)           4.625      9   97.03  <2e-16 ***
    ## s(Height)        8.965      9 1119.69  <2e-16 ***
    ## s(slope)         5.715      9   95.95  <2e-16 ***
    ## s(NorthVelocity) 7.218      9 1238.97  <2e-16 ***
    ## s(EastVelocity)  7.430      9  320.87  <2e-16 ***
    ## s(chlorA)        7.871      9  710.59  <2e-16 ***
    ## s(depth)         8.421      9 1646.20  <2e-16 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## R-sq.(adj) =  0.619   Deviance explained = 54.8%
    ## fREML = 2.3351e+05  Scale est. = 1         n = 170699

``` r
gratia::draw(BAM, residuals = TRUE)
```

![](5_GAMs_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

It seems like both methods end up overfitting the data, as they both
conclude that all covariates are important with lots of wiggles, which
is just biologically very unrealistic. This is probably due to
autocorrelation in the data, so the forward model selection with
cross-validation is the better way to do things.
