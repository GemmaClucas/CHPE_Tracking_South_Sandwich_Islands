...
This csv file has the data from all Ptts after limiting to just the chick-rearing trips (no pre-moult trips).
```{r}
...
What is the mean max distance from the colony across all trips?

```{r}
mean(trip_distances$Max_dist)
range(trip_distances$Max_dist)
median(trip_distances$Max_dist)
```
