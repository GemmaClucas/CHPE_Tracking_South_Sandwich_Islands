...
If trips are not being split up properly because there were no fixes within the buffer zone around the island, use this code to plot the problematic trip(s) and parse out sections of it to work out where the split should go. Change the number in ```trip_number``` for the trip that needs to be split, and add rows in increments, specified by ```a```, to visualise when the penguin returns to the colony.
...
Next, correct the trip numbers in the dataframe and replot the trips to make sure it's worked.
```{r}
