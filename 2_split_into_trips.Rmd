...
Plot the trips. Note the colour of the points denotes the speed over the previous 3 hours in kph.
```{r, message=FALSE}
...
Maybe a cut-off of something slightly above one is needed here? Although this penguin doesn't have much data, so it's probably not very representative.
...
1 kph seems like a good cut-off for this bird, which is the one we have the most data from.
...
1. I think I should go ahead and remove portions of the trips when the average speed was less than 1 kph over 3 hour windows.

2. Go through all individuals and delete the short trips which are clearly just the result of an inaccurate fix. Is there a rule of thumb to follow for keeping trips that look real (e.g. the have some wiggles in them?) and deleting the bad stuff?